...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($username) || empty($email) || empty($password)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT id FROM producer WHERE username = ?");
            $stmt->execute([$username]);
            if ($stmt->fetch()) {
                $error_message ="This username is already registered.";
            } else {
                //Check if email already exists
                $stmt = $pdo->prepare("SELECT id FROM producer WHERE email = ?");
                $stmt->execute([$email]);
                if ($stmt->fetch()) {
                    ...
                    $sql = "INSERT INTO producer (username, email, password) VALUES (?, ?, ?)";
                    $stmt = $pdo->prepare($sql);

                    //Execute the statement
                    if ($stmt->execute([$username, $email, $hashed_password])) {
                        //Successful, redirect to admin dashboard
                        header("Location: admin-dash.php?signup=success");
                        exit;
                    } else {
...
<!-- This is where the admin will be able to create new producer accounts -->
<div class="form-container">
    
    <div class="form">
        <h2>Create New Producer Account</h2>

        <form action="prod-acc-creation.php" method="POST">
